...
namespace kf
{
    template < int32_t DIM_X, int32_t DIM_Z, int32_t DIM_V, int32_t DIM_N > class UnscentedKalmanFilter : public KalmanFilter< DIM_X, DIM_Z >
    {
    public:
        ...
        /// function
        ///
        template < typename PredictionModelCallback > void predictUKF( PredictionModelCallback predictionModelFunc )
        {
        ...
        ///
        template < typename MeasurementModelCallback > void correctUKF( MeasurementModelCallback measurementModelFunc, const Vector< DIM_Z >& vecZ )
        {
            // self.x_a[:self.dim_x] = x
            // self.P_a[:self.dim_x, : self.dim_x] = P
            updateAugmentedStateAndCovariance();
    ...
    private:
        ...
        ///
        template < int32_t STATE_DIM > void calculateWeightedMeanAndCovariance( const Matrix< STATE_DIM, SIGMA_DIM >& sigmaX, Vector< STATE_DIM >& vecX, Matrix< STATE_DIM, STATE_DIM >& matPxx )
        {
            ...
            for ( int32_t i{ 1 }; i < SIGMA_DIM; ++i )
            {
                ...
                const Matrix< STATE_DIM, STATE_DIM > Pi{ m_weighti * devXi * devXi.transpose() };  // P_i = W[0, i] (Y[:, i] - \bar{y}) (Y[:, i] -
                                                                                                   // \bar{y})^T
        ...
        ///
        Matrix< DIM_X, DIM_Z > calculateCrossCorrelation( const Matrix< DIM_X, SIGMA_DIM >& sigmaX, const Vector< DIM_X >& vecX, const Matrix< DIM_Z, SIGMA_DIM >& sigmaY, const Vector< DIM_Z >& vecY )
        {
